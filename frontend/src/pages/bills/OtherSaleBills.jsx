...
const OtherSaleBills = () => {
  ...
  const renderPaginationLinks = () => {
    const totalPages = otherSaleBills?.totalPages;
    const paginationLinks = [];
    for (let i = 1; i <= totalPages; i++) {
      paginationLinks.push(
        <li key={i} onClick={ToDown}>
          <Link
            to={`/dashboard/other-sale?page=${i}`}
  ...
  return (
    <>
      ...
      {/* -------- PAGINATION -------- */}
      {otherSaleBills?.totalPages && otherSaleBills?.totalPages !== 1 ? (
        <section className="flex justify-center">
          <nav aria-label="Page navigation example">
            <ul className="flex items-center -space-x-px h-8 py-10 text-sm">
              <li>
                {otherSaleBills?.page > 1 ? (
                  <Link
                    onClick={ToDown}
                    to={`/dashboard/other-sale?page=${page - 1}`}
              ...
              <li>
                {otherSaleBills?.totalPages !== page ? (
                  <Link
                    onClick={ToDown}
                    to={`/dashboard/other-sale?page=${page + 1}`}
