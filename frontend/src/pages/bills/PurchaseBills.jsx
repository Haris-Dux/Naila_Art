...
import { getAllPurchasingHistoryAsync, validateOldSellerAsync } from '../../features/SellerSlice';
...
const PurchaseBills = () => {
    ...
    const handleChooseSeller = (sellerId) => {
        if (sellerId) {
            const sellerData = validateSeller?.oldSellerData.find((data) => data.id === sellerId);
        ...
        <>
            <section className='bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-600 mt-7 mb-0 mx-6 px-5 py-6 min-h-screen rounded-lg'>
                ...
                <div className="tabs flex justify-between items-center my-5">
                    ...
                    {/* OPEN MODAL BUTTON */}
                    <button onClick={openSearchModal} className="inline-block rounded-sm border border-gray-700 bg-gray-600 p-1.5 hover:bg-gray-800 focus:outline-none focus:ring-0">
            ...
            {/* ALL MODALS  */}
            {selectedCategory === 'Base' && <BaseModals isOpen={isOpen} closeModal={closeModal} sellerDetails={sellerDetails} />}
            {selectedCategory === 'Lace' && <LaceModal isOpen={isOpen} closeModal={closeModal} sellerDetails={sellerDetails} />}
            {selectedCategory === 'Bag/box' && <BagModal isOpen={isOpen} closeModal={closeModal} sellerDetails={sellerDetails} />}
            {selectedCategory === 'Accessories' && <AccessoriesModal isOpen={isOpen} closeModal={closeModal} sellerDetails={sellerDetails} />}
                            ...
                            {searchUser && (
                                <div className="search_user border-t flex justify-center flex-col pt-6 mt-6">
                                    ...
                                    {loading ? (
                                        ...
                                        <>
                                            {validateOldSeller.length > 0 && validateSeller && validateSeller?.oldSellerData && validateSeller?.oldSellerData.length > 0 ? (
                                                    ...
                                                    {validateSeller?.oldSellerData.map((seller) => (
                                                        <li key={seller.id}>
                                                            <button
                                                                ...
                                                                <span className='text-start'>{seller?.name}</span>
                                                                <span>{seller?.seller_stock_category}</span>
                                                                <span className='text-end'>{seller?.phone}</span>
                                                            </button>
                                                        </li>
                                                    ))}
                                                </ul>
                                            ) : (
                                                <p className="mt-4 text-gray-600 pl-4">No matching seller found.</p>
                                            )}
                                        </>
                                    )}
