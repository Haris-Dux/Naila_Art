...
export const getbPairDataAsync = createAsyncThunk("b_pair/getBpair" , async (data) => {
    try {
        const search =
        data?.search !== undefined && data?.search !== null 
            ? `&search=${data?.search}`
            : "";
        const response = await axios.post(`${getbPairUrl}?page=${data.page}&category=${data?.category}${search}`)
...
export const salebPairAsync = createAsyncThunk("b_pair/saleBPAir" , async (data) => {
    try {
